 ...
 *
 * <p>
 * <b>TUTORIAL: INTEGRATION TESTING PATTERNS</b>
 * <ul>
 * <li><b>@EmbeddedKafka:</b> Spins up a real, in-memory Kafka broker (and
 * Zookeeper)
 * locally on your machine during the test. No external infra required!</li>
 * <li><b>@SpringBootTest:</b> Loads the full Application Context, allowing us
 * to test the interaction between all beans (Service -> Producer ->
 * Kafka).</li>
 * <li><b>@DirtiesContext:</b> Ensures a fresh Spring context for this class,
 * preventing side-effects from other tests that might have modified shared
 * beans.</li>
 * </ul>
 */
@SpringBootTest
...
class KafkaPipelineIntegrationTest {
    ...
    /**
     * TUTORIAL: Testing asynchronous systems like Kafka is hard!
     * We use a {@link BlockingQueue} to "catch" messages as they arrive in our test
     * listener.
     * This allows our main test thread to wait (block) until the message is
     * received or a timeout occurs.
     */
    private static final BlockingQueue<ConsumerRecord<String, Object>> RECEIVED = new LinkedBlockingQueue<>();

    @Autowired
    private DataIngestionService dataIngestionService;

    /**
     * TUTORIAL: This is a dedicated "Test Consumer". It listens to the topic
     * we are testing and pipes every message into our BlockingQueue for
     * verification.
     */
    @KafkaListener(
    ...
    void ingestTransaction_shouldPublishToRawTopic() throws Exception {
        ...
        assertThat(record)
                .as("A record should be published to the raw-transactions topic")
                .isNotNull();
