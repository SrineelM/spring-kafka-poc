 ...
 *
 * <p>
 * <b>TUTORIAL: TESTING THE STRATEGY PATTERN</b>
 * <ul>
 *   <li><b>Objective:</b> We want to verify that the {@link DynamicPersistenceRouter}
 *       correctly selects the right storage engine based on what's available.</li>
 *   <li><b>Mocking:</b> We use {@code @Mock} to create "Stubs" for the real
 *       Spanner and H2 services. This ensures we are testing the ONLY the routing
 *       logic, not the actual database connections.</li>
 *   <li><b>Verification:</b> {@code verify(spannerService).save(...)} is how we confirm
 *       that the router actually called the specific instance we expected.</li>
 * </ul>
 */
...
class DynamicPersistenceRouterTest {
    ...
    @Mock
    private TransactionPersistencePort spannerService;

    @Mock
    private TransactionPersistencePort h2Service;
    ...
    void shouldRouteToSpannerWhenPresent() {
        ...
        DynamicPersistenceRouter router = new DynamicPersistenceRouter(List.of(spannerService, h2Service));

        Transaction transaction =
                Transaction.builder().transactionId("transaction-1").build();
