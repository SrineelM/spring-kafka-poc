 ...
 *
 * <p><b>TUTORIAL:</b> In enterprise systems, it's not enough that something failed or succeeded;
 * you must know exactly <em>when</em> and <em>why</em>. This service acts as an append-only log of
 * critical domain events.
 *
 * <p>Note the use of {@code @Transactional(propagation = Propagation.REQUIRES_NEW)}. This is
 * crucial! Even if the main business transaction rolls back (e.g., due to a database error), the
 * audit log of that attempt MUST still be persisted. This annotation suspends the current
 * transaction and starts a new, independent one just for the audit log.
 */
@Slf4j
@Service
