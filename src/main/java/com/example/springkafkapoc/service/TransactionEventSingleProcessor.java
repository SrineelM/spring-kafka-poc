 ...
 *
 * <p>
 * <b>TUTORIAL:</b> This is the main consumer. It listens to the incoming Kafka
 * topic
 * and processes records ONE AT A TIME. This provides the highest level of error
 * tracking
 * and transaction isolation, at the cost of some overall throughput compared to
 * batching.
 *
 * <p>
 * It demonstrates the <b>Transactional Outbox Pattern</b>:
 * 1. Read message.
 * 2. Write domain entity to DB.
 * 3. Write 'Outbox' event to DB.
...
public class TransactionEventSingleProcessor {
    ...
    public TransactionEventSingleProcessor(
        ...
        this.dlqCounter = Counter.builder("transaction.dlq.count")
    ...
    /**
     * TUTORIAL: {@code @RetryableTopic} provides non-blocking, delay-based retries
     * out of the box.
     * If this method throws an exception, Spring creates companion topics (e.g.,
     * topic-retry-0, topic-retry-1)
     * and forwards the message there automatically. It backs off exponentially.
     *
     * If all retries fail, it goes to the DLT (Dead Letter Topic).
     *
     * TUTORIAL: {@code @KafkaListener} marks this method as a Kafka Consumer.
     * By using {@code containerFactory = "kafkaListenerContainerFactory"}, we
     * ensure it uses
     * our custom interceptors and manual acknowledgement settings.
     */
    @RetryableTopic(
            attempts = "3",
    ...
    public void processTransaction(ConsumerRecord<String, TransactionEvent> record, Acknowledgment ack) {
        ...
        processingTimer.record(() -> {
            ...
            new TransactionTemplate(transactionManager).executeWithoutResult(status -> {
                ...
                auditService.recordProcessing(transactionId, accountId, record.partition(), record.offset());
                ...
                // 4. Audit success
                auditService.recordSuccess(transactionId, accountId, transactionPersistencePort.getStoreName());
            });
