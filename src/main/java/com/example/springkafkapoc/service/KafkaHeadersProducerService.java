 ...
 *
 * <p>
 * <b>TUTORIAL:</b> While the {@link DataIngestionService} handles the simple
 * data "push", this
 * service demonstrates the <b>Rich Messaging Patterns</b> required in
 * high-compliance or
 ...
 * <ul>
 * <li><b>Manual Header Injection:</b> Using {@code ProducerRecord} to add
 * metadata (Trace IDs,
 * Source System names) <i>outside</i> the core Avro data schema. This allows
 * infrastructure
 * to read metadata without deserializing the entire business payload.</li>
 * <li><b>Request-Reply (RPC):</b> Implementing a blocking-wait for an
 * asynchronous response.
 * One service sends a "Request" message, and wait on a "Reply" topic for a
 * "Response" from
 * another service.</li>
 * </ul>
 */
...
public class KafkaHeadersProducerService {
     ...
     *
     * <p>
     * Tutorial Tip: We use <b>ProducerRecord</b> for fine-grained control.
     * Headers allow you to pass routing or trace data <i>without</i>
     * modifying your Avro data schema.
     */
    public CompletableFuture<SendResult<String, TransactionEvent>> sendWithHeaders(
        ...
        if (correlationId != null) {
            record.headers()
                    .add(new RecordHeader(HEADER_CORRELATION_ID, correlationId.getBytes(StandardCharsets.UTF_8)));
        }
        record.headers()
                .add(new RecordHeader(HEADER_SOURCE_SYSTEM, SOURCE_SYSTEM_NAME.getBytes(StandardCharsets.UTF_8)));
    ...
    /**
     * Demonstrates the <b>Request-Reply (RPC)</b> pattern.
     * The method blocks until a response is received on a reply topic.
     *
     * <p>
     * Tutorial Tip: DO NOT use this for high-throughput flows.
     * It couples the producer to a specific consumer response, which
     * limits your horizontal scalability. It's best for "Confirm-before-Proceed"
     * loops.
     */
    public String sendAndAwaitReply(BigDecimal amount, String accountId) {
        ...
        Message<TransactionEvent> requestMessage = MessageBuilder.withPayload(event)
