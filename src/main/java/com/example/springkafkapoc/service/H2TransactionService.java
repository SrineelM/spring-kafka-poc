 ...
 * This is an <em>Adapter</em> in the ports-and-adapters model. It implements the
 * {@link TransactionPersistencePort} interface using an embedded H2 in-memory database. H2 needs
 * no external server, starts instantly, and wipes itself clean on each restart — making it ideal
 * for local development and CI pipelines.
 *
 * <p><b>WHY no {@code @ConditionalOnProperty} here?</b><br>
 * Unlike Spanner and AlloyDB which are conditional, H2 is always available. The
 * {@link DynamicPersistenceRouter} treats it as the default fallback. In a CI environment with
 * no cloud credentials, H2 ensures the application can still start and be tested.
 *
 * <p><b>PRO TIP:</b> The bean name {@code "h2TransactionService"} is explicit. If you inject
 * by name ({@code @Qualifier("h2TransactionService")}), you can force H2 even when Spanner is
...
public class H2TransactionService implements TransactionPersistencePort {
   ...
   *
   * <p>The mapper converts the persistence-agnostic {@link Transaction} domain model to a
   * {@link com.example.springkafkapoc.persistence.entity.TransactionEntity} before saving.
   * This prevents JPA annotations from leaking into the domain layer.
   */
  @Override
  ...
  public String getStoreName() {
    return "H2 In-Memory"; // Surfaced in audit logs and the DynamicPersistenceRouter selection logic
  }
}
