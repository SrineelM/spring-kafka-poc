 ...
 *
 * <p>
 * <b>TUTORIAL:</b> This service demonstrates <b>Interactive Queries (IQ)</b> in
 * Kafka Streams.
 * Traditionally, you aggregate data in a stream and write it to an external
 * database (like Spanner)
 * for an API to query. That's "Push" logic.
 *
 * <p>
 * IQ changes the game by allowing the API to query the <b>Active Processing
 * State Store</b>
 * directly. No external database is required for real-time dashboards or
 * lookups!
 *
 * <p>
 * <b>Key Architecture Tip:</b>
 * State stores are <b>local</b> to the processing instance that owns that key's
 * partition.
 * In a multi-node production cluster, your API front-end would need to perform
 * <b>RPC Routing</b>:
 * 1. Ask Kafka: "Which node currently owns the data for Account X?"
 * 2. Forward the HTTP request to that specific node.
 * 3. That node queries its local store and returns the result.
 */
@Slf4j
@Service
...
public class AnalyticsQueryService {
    ...
    public BigDecimal getDailyAccountTotal(String accountId) {
        ...
        ReadOnlyWindowStore<String, BigDecimal> store = streams.store(StoreQueryParameters.fromNameAndType(
                AnalyticsTopology.DAILY_ACCOUNT_AGGREGATES_STORE, QueryableStoreTypes.windowStore()));
