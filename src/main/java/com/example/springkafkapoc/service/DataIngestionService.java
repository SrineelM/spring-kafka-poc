 ...
 *
 * <p><b>TUTORIAL:</b> This is the "Front Door" of the application. It acts as the primary Producer
 * into the Kafka ecosystem. In a real-world scenario, this might be triggered by a REST endpoint or
 * a gRPC call.
 *
 * <p>We use <b>Avro</b> for serialization. Avro provides a strict schema (contract) between
 * services, which is essential in a production microservices environment. If the schema changes in
 * an incompatible way (e.g., a required field is removed), the build will fail, preventing
 * downstream consumers from crashing at runtime.
 */
@Slf4j
@Service
