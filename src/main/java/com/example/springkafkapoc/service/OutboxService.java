 ...
 *
 * <p>
 * <b>TUTORIAL:</b> The Outbox pattern is a vital microservices architecture
 * pattern used
 * to achieve atomic operations between a local database and an external message
 * broker (Kafka).
 *
 * <p>
 * Problem: You need to update a database AND send a Kafka message. If the DB
 * commits but Kafka
 * is down, the system is in an inconsistent state.
 *
 * <p>
 * Solution: Save the intended Kafka message into a local DB table (the
 * "Outbox") as part
 * of the SAME transaction that updates the business entity. Later, a separate
 * process (like
 * Debezium or a polling relayer) reads the outbox table and pushes to Kafka.
 */
@Slf4j
@Service
...
public class OutboxService {
     ...
     *
     * <p>
     * WHY: This is usually called from within an existing transaction (e.g., inside
     * the TransactionEventSingleProcessor).
     */
    @Transactional
    ...
    /**
     * Legacy helper for simpler calls where you don't want to build the model
     * manually.
     */
    @Transactional
     ...
     * (Note: Real implementation would have a 'findByProcessedFalse' repository
     * method)
     */
