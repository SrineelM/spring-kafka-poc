 ...
 * <b>Audit API</b>
 *
 * <p>This controller provides visibility into the lifecycle of transactions. In a production
 * system, auditing is crucial for debugging distributed race conditions and honoring SLAs.
 *
 * <p>Tutorial Tip: Notice how we return Domain Models (AuditLog) rather than persistence entities
 * (AuditLogEntity). This decouples our API from our database schema.
 */
@Slf4j
@RestController
...
public class AuditController {
  ...
  public ResponseEntity<List<AuditLog>> getTransactionAuditTrail(
  ...
  /**
   * Returns how many times a transaction was retried. A high retry count is an early warning signal
   * of a systemic issue.
   */
  ...
  /**
   * Shows which database backend is currently active. Returns "Cloud Spanner (with H2 fallback)" in
   * production, or "H2 In-Memory" in local/fallback mode.
   */
  @GetMapping("/storage-info")
  public ResponseEntity<Map<String, String>> storageInfo() {
    return ResponseEntity.ok(
        Map.of("activeStorage", persistenceRouter.getStoreName(), "status", "UP"));
  }
}
