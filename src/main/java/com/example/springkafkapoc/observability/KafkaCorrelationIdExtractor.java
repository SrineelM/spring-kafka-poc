 ...
 *
 * <p>Restores the tracking context from Kafka headers back into the application.
 *
 * <p>Tutorial Tip: This is part of <b>Full Lifecycle Traceability</b>. When a Kafka record arrives,
 * we extract the "correlationId" from its headers and set it in the <b>ThreadLocal</b>. Now, every
 * {@code log.info()} call in your consumer will automatically show which HTTP request triggered
 * this processing.
 */
@Slf4j
@Component
public class KafkaCorrelationIdExtractor<K, V> implements RecordInterceptor<K, V> {
  ...
  /**
   * Called AFTER the listener method returns. We MUST clear the context so it doesn't leak into the
   * next record processed by this thread.
   */
  @Override
