 ...
 * <ul>
 *   <li>{@code FAIL} — Re-throw: halt the stream thread and alert loudly. Use this when 100%
 *       data fidelity is required and you cannot tolerate silent data loss.
 ...
 * In production, before returning {@code CONTINUE}, produce the raw {@code record.value()} bytes
 * to a dedicated poison-pill topic (e.g., {@code raw-transactions-STREAMS-POISON}) using an
 * independent {@code KafkaProducer}. This gives operations a chance to inspect and replay the
 * bad record after fixing the schema or producer.
 */
@Slf4j
public class StreamsDeserializationErrorHandler implements DeserializationExceptionHandler {
   ...
   *
   * <p>We log the full location (topic, partition, offset) so operations can use
   * {@code kafka-console-consumer --topic X --partition Y --offset Z} to retrieve and inspect
