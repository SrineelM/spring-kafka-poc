 ...
 *
 * <p><b>TUTORIAL:</b> This is the core "Intelligence" of the system, implemented using <b>Kafka
 * Streams</b>. Unlike the standard Consumer API which processes records one-by-one, Kafka Streams
 * allows you to define a functional "Processing Topology" (a graph of operations) that the engine
 * executes with built-in scalability and fault tolerance.
 *
 * <p>This class demonstrates several advanced stream-processing patterns:
 *
 * <ul>
 *   <li><b>Stream-Table Join:</b> Enriches real-time transaction events with reference data
 *       (Account names).
 *   <li><b>Branching:</b> Dynamically routes messages to different topics based on business logic
 *       (Fraud/High Value).
 *   <li><b>Stateful Aggregation:</b> Computes rolling totals (windows) for account activity.
 *   <li><b>Interactive Queries:</b> Exposes the results of these computations via Local State
 *       Stores, which can be queried directly by our REST controllers (see {@code
...
public class AnalyticsTopology {
  ...
  public void buildPipeline(StreamsBuilder builder) {
    ...
    // database table.
    GlobalKTable<String, String> accountTable =
        builder.globalTable(
    ...
    KStream<String, TransactionEvent> enrichedStream =
    ...
    enrichedStream
        ...
        .defaultBranch(
            Branched.withConsumer(
                stream ->
                    stream.to(
                        "normal-transactions", Produced.with(Serdes.String(), transactionSerde))));
    ...
    enrichedStream
        ...
        .aggregate(
            ...
            Materialized.<String, BigDecimal, WindowStore<Bytes, byte[]>>as(
                    DAILY_ACCOUNT_AGGREGATES_STORE)
                .withKeySerde(Serdes.String())
                .withValueSerde(bigDecimalSerde))
        .toStream()
        .map(
            (windowedKey, totalAmount) -> {
  ...
  /**
   * Helper to build a Serde for BigDecimal, which isn't provided by default. We serialize
   * BigDecimals as UTF-8 strings for maximal compatibility.
   */
