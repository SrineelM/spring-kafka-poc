 ...
 *
 * <p>
 * Centralizes the creation of Producer/Consumer factories and standard
 * templates.
 *
 * <p>
 * Tutorial Tip: This class is the "Heart" of the Kafka integration. It handles:
 * <ul>
 * <li><b>Idempotency:</b> Enabled by default to prevent duplicate data for
 * retries.</li>
 * <li><b>Serialization:</b> Uses Apache Avro for strict schema
 * enforcement.</li>
 * <li><b>Observability:</b> Hooks in Correlation ID interceptors for
 * tracing.</li>
 * </ul>
 */
@Slf4j
...
public class KafkaCoreConfig {
    ...
    /**
     * TUTORIAL: The {@code ProducerFactory} defines how to create Kafka producers.
     * We use a {@code DefaultKafkaProducerFactory} supplied with our common
     * properties.
     * This is the {@code @Primary} factory, meaning it will be used by default when
     * injecting a ProducerFactory or building standard templates.
     */
    @Bean
    @Primary
    ...
    /**
     * TUTORIAL: A specialized producer factory that overrides the default
     * partitioner.
     * We attach the {@code HighValueTransactionPartitioner} so that transactions
     * over
     * a certain value are explicitly routed to dedicated metrics partitions,
     * optimizing
    ...
    public ProducerFactory<String, TransactionEvent> customPartitionerProducerFactory() {
        ...
        props.put(ProducerConfig.PARTITIONER_CLASS_CONFIG, HighValueTransactionPartitioner.class.getName());
    ...
    /**
     * TUTORIAL: The {@code KafkaTemplate} simplifies sending messages to Kafka
     * topics.
     * It wraps the {@code ProducerFactory} and provides high-level operations like
     * {@code send(topic, key, value)}. It behaves much like Spring's JdbcTemplate
     * or RestTemplate.
     */
    @Bean
    @Primary
    ...
    /**
     * TUTORIAL: The {@code ConsumerFactory} creates Kafka consumers. Similar to the
     * producer,
     * it defines the core connection and deserialization rules.
     * Notice we configure it to deserialize Avro back into specific Java classes
     * natively.
     */
    @Bean
    ...
    /**
     * TUTORIAL: The {@code ConcurrentKafkaListenerContainerFactory} is the engine
     * behind
     * {@code @KafkaListener} annotations. It spawns the consumer threads.
     * We customize it here to:
     * 1. Add a global error handler for dealing with Exceptions during processing.
     * 2. Insert our Correlation ID extractor before our business logic runs.
     * 3. Set Manual acknowledgement mode so we control exactly when a message is
     * marked 'read'.
     */
    @Bean
    ...
    /**
     * TUTORIAL: Similar to the above, but specifically configured for batch
     * processing.
     * Useful when processing lots of small events simultaneously (e.g., buffering
     * writes to BigQuery).
     */
    @Bean
    ...
    public DefaultErrorHandler errorHandler(KafkaTemplate<String, TransactionEvent> template) {
        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(
                template, (r, e) -> new org.apache.kafka.common.TopicPartition(r.topic() + ".DLT", r.partition()));
